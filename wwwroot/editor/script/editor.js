...
function loadDatum(path) {
		...
		$.each(currentNode, function (i, o) {
			if (isObject(o)) {
				editor.append("<div class=\"table-row\">" +
							  ...
							  "<button class='delete-row'><i class=\"material-icons\" style=\"vertical-align: middle;\">remove_circle</i>Eliminar</button>" +
							  "</div>");
			} else {
				editor.append("<div class=\"table-row\">" +
							  "<input class=\"key-input\" readonly value=" + i + "> : " +
	...
	if (!$('.new').length) {
		...
		editor.append("<div class=\"table-row\" id='new-row'>" +
					  "<button class=\"new\" ><i class=\"material-icons\" style=\"vertical-align: middle;\">add_circle</i>Nuevo...</button> " + "<ul class='menu' id='new-option'>" +
